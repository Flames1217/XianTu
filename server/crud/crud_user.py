...
async def ensure_admin_account_exists():
    ...
    if not settings.DEFAULT_ADMIN_USERNAME or not settings.DEFAULT_ADMIN_PASSWORD:
        raise RuntimeError(
            "CREATE_DEFAULT_ADMIN=true 时必须配置 DEFAULT_ADMIN_USERNAME 和 DEFAULT_ADMIN_PASSWORD"
        )
    ...
    if not super_admin:
        await AdminAccount.create(
            user_name=settings.DEFAULT_ADMIN_USERNAME,
            password=security.get_password_hash(settings.DEFAULT_ADMIN_PASSWORD),
...
async def get_admin_by_id(admin_id: int) -> Optional[AdminAccount]:
    return await AdminAccount.get_or_none(id=admin_id)
