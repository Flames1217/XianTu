...
async def create_spirit_root_endpoint(
    spirit_root: schema.SpiritRootCreate,
...
async def get_spirit_roots_endpoint():
    ...
    return {"items": spirit_roots, "total": len(spirit_roots)}
...
async def update_spirit_root_endpoint(
    spirit_root_id: int,
    spirit_root: schema.SpiritRootUpdate,
    current_admin: AdminAccount = Depends(deps.get_super_admin_user),
):
    """更新核心灵根"""
...
async def delete_spirit_root_endpoint(
    spirit_root_id: int,
    current_admin: AdminAccount = Depends(deps.get_super_admin_user),
):
    """删除核心灵根"""
