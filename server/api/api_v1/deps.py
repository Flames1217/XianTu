...
async def get_current_user(token: str = Depends(reusable_oauth2)) -> PlayerAccount:
    try:
        ...
        if payload.get("account_type") != "player":
            raise ValueError("wrong authentication domain")
        player_id = int(payload["sub"])
    except (jwt.JWTError, ValidationError, ValueError, KeyError, TypeError):
        raise HTTPException(
    ...
    user = await crud_user.get_player_by_id(player_id=player_id)
    if not user:
...
async def get_current_admin_direct(authorization: Optional[str] = Header(None)) -> AdminAccount:
    ...
    admin = await crud_user.get_admin_by_id(admin_id=admin_id)
    if not admin:
...
async def get_current_admin(token: str = Depends(admin_oauth2)) -> AdminAccount:
    ...
    try:
        ...
        if payload.get("account_type") != "admin":
            raise ValueError("wrong authentication domain")
        admin_id = int(payload["sub"])
    except (jwt.JWTError, ValidationError, ValueError, KeyError, TypeError) as e:
...
async def get_current_active_user_optional(token: Optional[str] = Depends(reusable_oauth2_optional)) -> Optional[PlayerAccount]:
    ...
    try:
        ...
        if payload.get("account_type") != "player":
            return None
        user = await crud_user.get_player_by_id(player_id=int(payload["sub"]))
        return user
    except (jwt.JWTError, ValidationError, ValueError, KeyError, TypeError):
        return None
...
async def get_current_super_admin_optional(token: Optional[str] = Depends(admin_oauth2_optional)) -> Optional[AdminAccount]:
    ...
    try:
        ...
        if payload.get("account_type") != "admin":
            return None
        admin = await crud_user.get_admin_by_id(admin_id=int(payload["sub"]))
